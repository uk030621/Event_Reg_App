  ...
  return (
    ...
    >
      ...
      <p className="ml-3 text-base">
        Venue: <span className="font-bold text-sm">TBA</span>
      </p>
      <p className="ml-3 text-base ">
        Date: <span className="font-bold mr-4 text-sm">TBA</span>Time:{" "}
        <span className="font-bold mr-4 text-sm">TBA</span>
      </p>
      <Link
        className="text-slate-500 ml-3 text-sm mt-2 cursor-pointer text-decoration-line: underline underline-offset-4"
        href="https://thursday-club-blog.vercel.app/"
      >
        Back to blog
      </Link>
      ...
      <h2 className="text-base mt-6 mb-3 font-bold ml-3">
        Registered
        <span className="text-base ml-3 font-normal">
          {loading
            ? "(Loading...)"
            : `(${users.length} ${
                users.length === 1 ? "attendee" : "attendees"
              })`}
        </span>
      </h2>
            ...
            {editingUserId === user._id ? (
              ...
              >
                <input
                  ...
                  value={editingName}
                  onChange={(e) =>
                    setEditingName(capitalizeWords(e.target.value))
                  }
              ...
              <>
                <div className="flex items-center justify-between">
                  <span className="text-sm" style={{ marginRight: "20px" }}>
                    {user.name}
                  </span>
